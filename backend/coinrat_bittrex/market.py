...
class BittrexMarket:
    ...
    def buy_max_available(self, pair: MarketPair) -> str:
        ...
        can_buy = (balance.available_amount / tick.average_price) * Decimal(1 - self.transaction_fee)
    ...
    def sell_max_available(self, pair: MarketPair) -> str:
        ...
        market = self.format_market_pair(pair)
        tick = self.get_last_ticker(market)
        can_sell = (tick.average_price * balance.available_amount) * Decimal(1 - self.transaction_fee)
